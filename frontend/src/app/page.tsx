...
const DEMO_REPORT: ReportData = {
  ...
  analysis: {
    change_summary: 'Apple 이 iPhone 메인 페이지 Hero 섹션을 전면 개편했습니다. 헤드라인이 "Hello, Apple Intelligence."로 교체되고, JSON-LD Product 스키마에 speakable 및 FAQPage 타입이 추가되어 AI 검색 노출 구조가 강화됐습니다.',
    ...
    insights: [
      'AI: iPhone 페이지 JSON-LD 에 speakable + FAQPage 타입 추가 — Google AI Overview 스니펫 노출 가능성 상승',
      'Brand: Hero H1 "Hello, Apple Intelligence." 교체 — og:title·meta description 에도 동시 반영',
      'UX: Hero 이미지를 라이프스타일 그룹 이미지로 교체, 타깃층 확장 의도 명확',
      'Pricing: offers 스키마 내 price 필드를 월 할부 기준으로 변경',
      'SEO: FAQPage 스키마 추가로 음성 검색·AI 답변 최적화',
    ],
    action_items: [
      '🚨 [즉시] samsung.com 에 speakable + FAQPage JSON-LD 스키마 적용',
      '⚠️ [1 주 내] Galaxy AI 페이지 H1 슬로건 단일화',
      '👀 [2 주 내] Galaxy 구매 페이지 CTA sticky 상단 고정',
    ],
    priority_label: 'Critical', functional_area: 'SEO·AI 인덱싱 · 헤드라인·슬로건',
    category_insights: {
      'SEO·AI 인덱싱':  { status: '위험', summary: 'Apple speakable + FAQPage 완비. Samsung 미적용.', apple_score: 9, samsung_score: 4, improvement_points: ['speakable 스키마 즉시 적용', 'FAQPage 스키마 추가'] },
  ...
  data_changes: [
    { url: 'https://www.apple.com/iphone/', site: 'Apple', tier: 'Tier 0', added: 82, removed: 34, title_changed: true, severity: 'Critical', severity_score: 88, change_types: ['SEO·AI 인덱싱', '헤드라인·슬로건'] },
    { url: 'https://www.apple.com/iphone-17-pro/', site: 'Apple', tier: 'Tier 1', added: 43, removed: 8, title_changed: false, severity: 'High', severity_score: 48, change_types: ['비주얼·미디어', '본문·기능 설명'] },
  ],
  content_changes: [{ url: 'https://www.apple.com/iphone/', site: 'Apple', tier: 'Tier 0' }],
  total_changed_urls: 2,
};
...
const DEMO_SNAPSHOT: ReportData = {
  ...
  analysis: {
    ...
    samsung_comparison: 'Apple 은 speakable + FAQPage + BreadcrumbList 스키마를 전 페이지에 완비한 반면, samsung.com/sg 는 기본 Product 스키마만 적용된 페이지가 약 70% 에 달합니다.',
    insights: [
      'AI: Apple iPhone 전 제품 페이지 speakable + FAQPage 스키마 완비 — Google AI Overview 내 노출 구조 선점',
    ...
    action_items: [
      '🚨 [즉시 - 개발 2 일] samsung.com/sg 전 스마트폰 페이지에 speakable + FAQPage JSON-LD 스키마 적용',
      '⚠️ [1 주 내] Galaxy AI 랜딩 H1 을 "Galaxy AI. 당신의 일상을 바꾸는 AI." 형태로 단일화',
      '⚠️ [1 주 내] Galaxy 구매 페이지 CTA sticky 상단 고정으로 변경',
      '👀 [2 주 내] Galaxy S26 Hero 에 월 할부 기준 가격 추가 표기',
      '👀 [2 주 내] 주요 제품 이미지 alt 텍스트에 AI 기능 키워드 포함',
    ],
    priority_label: 'High', functional_area: 'SEO·AI 인덱싱 · CTA·구매 흐름 · 헤드라인·슬로건',
    category_insights: {
      'SEO·AI 인덱싱':  { status: '위험', summary: 'Apple speakable + FAQPage + Product + BreadcrumbList 스키마 전 페이지 완비. Samsung 전체의 70% 가 기본 Product 스키마만 적용.', apple_score: 9, samsung_score: 3, improvement_points: ['speakable 스키마 즉시 적용', 'FAQPage 스키마 추가', 'BreadcrumbList 완비'] },
